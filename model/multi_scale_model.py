...
class MSResNet(nn.Module):
    def __init__(self, input_channel, layers=[1, 1, 1, 1], num_classes=10):
        ...
        # maxplooing kernel size: 16, 11, 6,(18,14,9),
        self.maxpool3 = nn.AvgPool1d(kernel_size=9, stride=1, padding=0)
        ...
        self.maxpool5 = nn.AvgPool1d(kernel_size=14, stride=1, padding=0)
        ...
        self.maxpool7 = nn.AvgPool1d(kernel_size=9, stride=1, padding=0)

        # self.drop = nn.Dropout(p=0.2)
        self.fc = nn.Linear(128*1+16, num_classes)
    ...
    def forward(self, x0):
        x_clinical = x0[:,290:306]
        featureClinical  = self.encoder(x_clinical)
        x0 = x0[:,2:290]
        x0 = x0.reshape(-1,1,288)
        x0 = self.conv1(x0)
        x0 = self.bn1(x0)
        x0 = self.relu(x0)
        x0 = self.maxpool(x0)
        ...
        featureCGM = torch.cat([y], dim=1)
        #out = z
        featureCGM = featureCGM.reshape(-1,128)
        featureFusion = torch.cat([featureCGM,featureClinical],dim=1)
        #out = self.drop(out)
        output = self.fc(featureFusion)

        return output, featureFusion
